...
//==================================================================================================//	
	...
	public void setup6() throws Throwable 
	{
		...
		//Open URL	
		Thread.sleep(3000);
		driver.get(BaseTest.getExcelControl("Authenticate", getControlRow(), 3));
		driver.manage().window().maximize();
		
		//Enter Username and Password from Excel
		Thread.sleep(1000);
		BaseTest.click(g.getMatriID());
		BaseTest.typeData(g.getMatriID(), getExcelControl("Authenticate", getControlRow(), 0));
		Thread.sleep(1000);
		BaseTest.click(g.getPasswordClear());
		BaseTest.typeData(g.getPassword(), getExcelControl("Authenticate", getControlRow(), 1));
		...
		//open regional URL
			...
			URL=URL+BaseTest.getExcelControl("Authenticate", getControlRow(), 4);
		...
		//To find number of rows and columns
			FileInputStream fis = new FileInputStream("D:\\Java\\Window7\\First\\matrimonyart\\Excel\\ControlSection.xlsx");
			 Workbook wb = WorkbookFactory.create(fis);
			 System.out.println("workbook is created");
			 Sheet sh = wb.getSheet(getExcelControl("Authenticate", getControlRow(), 2));
...
//			}
			...
			driver.findElement(By.xpath("//*[@id='visaNri_vpc_CardNum']")).sendKeys(getExcelControl("Payment", 1, 1));
			...
			s.selectByVisibleText(getExcelControl("Payment", 2, 1));
			s = new Select(cExpiryYear);
			s.selectByVisibleText(getExcelControl("Payment", 3, 1));
			int passcode = Integer.parseInt(getExcelControl("Payment", 4, 1));
			driver.findElement(By.xpath("//*[@id='visaNri_vpc_CardSecurityCode']")).sendKeys(getExcelControl("Payment", 4, 1));
			Thread.sleep(5000);
			driver.findElement(By.xpath("(//*[@tabindex='13'])[1]")).click();
			
			
			Thread.sleep(20000);
			WebElement PayOptionAmt=null;
			try {
				 PayOptionAmt = driver.findElement(By.xpath("//*[@class='disblk padb10 font12']"));
			} catch (Exception e) {
				PayOptionAmt = driver.findElement(By.xpath("//*[@class='disblk padb10 font13']"));
			}
